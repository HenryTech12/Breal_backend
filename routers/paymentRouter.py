...
def initialize_payment(data: paymentSchema.PaymentSchema, db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
...
def verify_payment(data: paymentSchema.VerifyPayment, db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
