...
def fetch_client_profile(email: str = Depends(securityConfig.get_current_user), db: Session = Depends(database.get_db)):
    return clientService.get_client_profile(email,db)

@client_router.patch('/clients/profile')
def update_client_profile(data: clientSchema.UpdateClientProfileSchema, email: str = Depends(securityConfig.get_current_user), db: Session = Depends(database.get_db)):
