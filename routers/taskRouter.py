...
def create_task(project_id, data: taskSchema.TaskSchema, db: Session = Depends(database.get_db)):
    return taskService.create_task(data,id,db)
    
@task_router.get('/projects/{project_id}/tasks')
def get_all_task(project_id,db: Session = Depends(database.get_db)):
    return taskService.get_tasks(project_id,db)

@task_router.patch('/task/{task_id}')
def update_task(duedate: datetime, task_id, db: Session = Depends(database.get_db)):
    data = {
        "duedate": duedate
    }
...
def complete_task(task_id, db: Session = Depends(database.get_db)):
