...
def create_consultations(data: consultationSchema.ConsultationSchema, db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
    return consultationService.create_consultation(email,data,db)
    
@consultation_router.get('/consultations/my')
def get_my_consultation(db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
    return consultationService.get_my_consultations(email,db)

@consultation_router.get('/admin/consultations')
def get_all_consultation(db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
...
def update_consultation(consultation_id: int, data: consultationSchema.UpdateConsultation, db: Session = Depends(database.get_db), email: str = Depends(securityConfig.get_current_user)):
    user_data = userService.get_user(email,db)
    if user_data.role == userSchema.UserRole.admin:
